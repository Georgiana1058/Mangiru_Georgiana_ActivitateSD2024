...
int calculHash(int cod, const char* nume, int dimensiune) {
	//se poate face aici orice->sa se creeze functia hash ca sa fie unica,modul de calcul sa fie unic
	if (dimensiune > 0) {
...
void inserareGalerieInTabela(HashTable hash, Galerie galerie) {
	if (hash.dim > 0) {
		...
		if (hash.vector[pozitie] != NULL) {//aici if este aici doar pt a prezenta situatia de coliziune
			inserareLaFinal(&(hash.vector[pozitie]), galerie);

		}
		else {//acelasi lucru ca mai sus
			inserareLaFinal(&(hash.vector[pozitie]), galerie);
		}
	}
}
void afisareGalerie(Galerie g) {
	printf("%d. Intrare la galeria%s costa %f\n", g.cod, g.nume, g.pretIntrare);
}
