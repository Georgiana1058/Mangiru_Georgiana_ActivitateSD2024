...
void afisareAeroport(struct Aeroport* aeroport) {
	if (aeroport != NULL) {
		printf("id.%d %s are %d terminale.\t", aeroport->id, aeroport->nume, aeroport->nrTerminale);
	}
...
//functia care returneaza un aeroport dupa id cautat in matrice
struct Aeroport* cautatAeroportID(struct Aeroport** matrice, int* nrColoane, int nrLinii, int Id) {
	for (int i = 0; i < nrLinii; i++)
	{
		for (int j = 0; j < nrColoane[i]; j++) {
			if (matrice[i][j].id == Id) {
				return&(matrice[i][j]);//returneaza pointerul trebuie sa obtinem adresa acelui obiect
			}
		}
	}
	return NULL;
...
void main() {
	//declaram matricea in care salvam aeroporturile
	...
	printf("\n\n\n");
	afisareAeroport(cautatAeroportID(aeroporturi, nrColoane, nrLinii, 7));
